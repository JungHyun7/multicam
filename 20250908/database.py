...
class MyDB:
    ...
    # 입력 받을 데이터는 데이터베이스 서버의 정보 -> 기본값 설정(로컬 PC의 데이터베이스 정보)
    def __init__ (
        self,
        _host = '127.0.0.1',
        _port = 3306,
    ...
    def sql_query(
        ...
        *_data_list
        ...
        # _data_list는 인자의 개수를 가변으로 갖는다. 개수가 0개면 ()을 생성
        
        # 데이터베이스 서버와의 연결(_db 변수를 생성하여 연결)
        # pymysql.connet() 함수는 입력값이 서버의 정보 -> 서버의 정보는 self.host, self.port, self.user, self.pw, self.db_name 저장되어 있다.
        try:
            # self._db라는 변수를 확인
            self._db
