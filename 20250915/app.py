...
def index():
    ...
    # 하위의 데이터 30개 필터
    df = df.tail(30)
    # index.html에 table에 컬럼의 이름들을 사용하기 위해 list형태의 columns를 생성
    cols = list(df.columns)
    values = df.to_dict(orient='records')  #oriendt='records' --> 리스트 안에 딕셔너리 형태로 만들어줌
    # table에 데이터의 값들을 [{}, {}, {}] 형태로 생성
    # chart에서 사용할 x측 데이터와 y축의 데이터를 list형태로 생성
    x = df['Date'].to_list()
    y = df['Volume'].to_list()
    # cols와 vlaues를 이용하여 index_html에서 table태그를 생성
